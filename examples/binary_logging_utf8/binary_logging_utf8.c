/*
 * Binary Logging UTF-8 Example
 *
 * This example demonstrates using clogging binary_logging with UTF-8 strings.
 * Binary logging writes messages in a binary format suitable for post-processing.
 * Compile with: cmake -B build -DCLOGGING_USE_UTF8_STRINGS=ON
 * Build with: cmake --build build
 * Run with: ./build/examples/binary_logging_utf8/binary_logging_utf8
 */
...
int main(void) {
  printf("Binary Logging UTF-8 Example\n");
  printf("============================\n\n");

  /* Initialize binary logging to write to stdout (file descriptor 1) */
  ...
  printf("\nBinary logging complete. Output was written in binary format.\n");
